...
bool NNUE::load(const std::string& path) {
    ...
    // First stage weights
    w_acc.resize(static_cast<size_t>(FEATURE_DIM) * 2 * ACC_UNITS);
    in.read(reinterpret_cast<char*>(w_acc.data()),
            w_acc.size() * sizeof(Weight));
    if (!in) return false;
    ...
    in.read(reinterpret_cast<char*>(w_fc1.data()),
            w_fc1.size() * sizeof(Weight));
    if (!in) return false;
    ...
    in.read(reinterpret_cast<char*>(w_fc2.data()),
            w_fc2.size() * sizeof(Weight));
    if (!in) return false;
...
inline void NNUE::add_feature(int feature_index, int sign) {
    ...
    const Weight* wf = &w_acc[static_cast<size_t>(feature_index) * stride];
...
int NNUE::evaluate() const {
    // Stage 1: clipped ReLU on accumulators, produce 512 activations
    ...
    // Stage 2: FC1 2*ACC_UNITS -> HIDDEN1
    Accum h1[HIDDEN1];
    for (int o = 0; o < HIDDEN1; ++o) {
        Accum sum = bias_fc1[o];
        const Weight* w_row = &w_fc1[o * 2 * ACC_UNITS];
        for (int i = 0; i < 2 * ACC_UNITS; ++i) {
            sum += static_cast<Accum>(w_row[i]) *
                   static_cast<Accum>(act[i]);
        }
        if (sum < 0) sum = 0;
        if (sum > RELU_CLIP) sum = RELU_CLIP;
        h1[o] = sum;
    }

    // Stage 3: FC2 HIDDEN1 -> HIDDEN2
    Accum h2[HIDDEN2];
